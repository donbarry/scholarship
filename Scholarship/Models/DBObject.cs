    ...
    public class DBObject
    {
        ...
        public ScholarshipData GetScholarshipData(string fundAcct, string scholarNum)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                data.financialneed =  aScholarship.SCHLR_USER_VARBL4.Length==0 ? "" : "<b>Financial Need : This scholarship requires a student to have a Financial Need to be eligible to receive the scholarship. In order to establish \"Financial Need\", a student must file the Free Application for Federal Student Aid (FAFSA). You can file the FAFSA at <a target='_blank' href=\"http://www.fafsa.ed.gov/\">www.fafsa.ed.gov</a>.";
        ...
        public List<Scholarship> GetScholarships(SearchObject searchObject)
        {
            ...
            sqlstr += "LEFT OUTER JOIN Summit.USER_CODE su on (s.audit_tran_id=su.parent_audit_tran_id and (su.USER_GRP='SCHMJ' or su.USER_GRP='SCHYR'))";
            sqlstr += "LEFT OUTER JOIN UHELP.USER_CD uu on su.USER_CD=UU.USER_CD ";
            sqlstr += " WHERE s.SCHLR_USER_VARBL2 = 'Y' and f.FUND_OPEN_ATTRB='O' AND rownum<1000 "; //explain this number later. choosing 3 cols also allows me have DISTINCT
            ...
            {
                sqlstr += " and f.FUND_COLL_ATTRB like :college "; //no need regex since we have dropdown for these
